...
export class Option {
  ...
  constructor (opt: {
    ...
    this.lib.silent = opt.silent
  }

  match (key: keyof Options) {
    this.key = key
    this.isMatch = false
    if (this.isURI) {
  ...
  toBuffer () {
    if (!this.isMatch) return this
    this.value = this.lib.buffer(this.value, this.authoredKey)
    return this
  }

  toFunction () {
    if (!this.isMatch) return this
    this.value = this.lib.function(this.value, this.authoredKey)
    return this
  }

  toFamily () {
    if (!this.isMatch) return this
    this.value = this.lib.family(this.value, this.authoredKey)
    return this
  }

  toPkFactory () {
    if (!this.isMatch) return this
    this.value = this.lib.pkFactory(this.value, this.authoredKey)
    return this
  }

  toPromiseLibrary () {
    if (!this.isMatch) return this
    this.value = this.lib.promiseLibrary(this.value, this.authoredKey)
    return this
  }
 ...
 /**
   ...
   */
  assign (parent: any, key?: keyof Options) {
    if (this.shouldUpdateValue) {
   ...
   */
  objectAssign (parent: any, key?: keyof Options) {
    if (this.shouldUpdateValue) {
      const resolveKey = key || this.key
      if (typeof resolveKey !== 'undefined') {
   ...
   */
  concat (parent: any, key?: keyof Options) {
    if (this.shouldUpdateValue) {
      const resolveKey = key || this.key
      if (typeof resolveKey !== 'undefined') {
   ...
   */
  push (parent: any, key?: keyof Options) {
    if (this.shouldUpdateValue) {
      const resolveKey = key || this.key
      if (typeof resolveKey !== 'undefined') {
        if (Array.isArray(parent[resolveKey])) {
